...
open class SCLAlertView: UIViewController {
    ...
    let kWindowWidth: CGFloat = 260.0
    ...
    let lowColor = UIColor(red: 46/255, green: 204/255, blue: 113/255, alpha: 1.0)
    let highColor = UIColor(red: 231/255, green: 76/255, blue: 60/255, alpha: 1.0)

    
    // Font
    //let kDefaultFont = "HelveticaNeue"
    let kDefaultFont = "Dubai"
    
    
   // let kButtonFont = "HelveticaNeue-Bold"
    let kButtonFont = "Dubai"
    
    
    // UI Colour
    var viewColor = UIColor()
    var pressBrightnessFactor = 0.75
    ...
    override open func viewWillLayoutSubviews() {
        ...
        for btn in buttons {
            if btn.tag == 250 {
                btn.frame = CGRect(x:110, y:y - 2, width:37, height:37)
                btn.setImage(UIImage(named:"urgencycancelbtn"), for: UIControlState())
                btn.backgroundColor = UIColor.clear
                btn.setBackgroundImage(UIImage(named:"urgencycancelbtn"), for: UIControlState.selected)
                // contentView.frame = CGRect(x:x, y:y, width:kWindowWidth, height:windowHeight + 08)
    ...
    open func addButton(_ title:String,tag : Int, action:@escaping ()->Void)->SCLButton {
        let btn = addButton(title)
       btn.tag = tag
...
//        self.circleBG.frame.origin.y = newBallViewFrameY
    ...
    open func showTitle(_ title: String, subTitle: String, duration: TimeInterval?, completeText: String?, style: SCLAlertViewStyle, colorStyle: UInt?=0x000000, colorTextButton: UInt?=0xFFFFFF, circleIconImage: UIImage? = nil) -> SCLAlertViewResponder {
        ...
        // Done button
        if showCloseButton {
            let txt = completeText != nil ? completeText! : "Dismiss"
        ...
        for btn in buttons {
            if btn.tag == 50 {
            btn.backgroundColor = self.lowColor
            }
           else if btn.tag == 100
            {
            btn.backgroundColor = self.mediumColor
            }
            else if btn.tag == 150 {
            btn.backgroundColor = self.highColor
            }
